...
class Admin extends CI_Controller {
    ...
    public function deleteGuide($id){
        ...
        public function deleteLanguage($id){
            if(!empty($id) && isset($id)){
                  $data=$this->modAdmin->deleteLanguage($id);
                  if ($data) {
                    $this->session->set_flashdata('class','alert-success');
                        $this->session->set_flashdata('message','Guide Delated');
                        redirect('admin/viewLanguage');
        ...
        public function editBlogPDF($id){
           ...
           $data['id']=$this->input->post('id',true); 
                ...
                if (!empty($data['package_name'])&& isset($data['body'])&& isset($data['id'])) {
                    ...
                    if (!$this->upload->do_upload('img')) {
                        $error=$this->upload->display_errors();
                        setFlashData('alert-danger','$error','admin/viewBlog');
                    }
                    else{
                        $fileName=$this->upload->data();
                        $data['img']=$fileName['file_name'];
                ...
                redirect('admin/viewBlog');
                        ...
                        setFlashData('alert-danger','You can not updated now','admin/viewBlog');
                    }
                    # code...
                }
                else{
                    setFlashData('alert-danger','Please Check Every Field','admin/viewBlog');
...
$data['payment_methood']= $this->input->post('payment_methood','true');
    ...
    $addptype_name=$this->modAdmin->checkptype_nameExistencs($data);
                    if ($addptype_name->num_rows()>0) {
                        ...
                        redirect('admin/addLanguageType');
    ...
    public function viewLanguageType(){
    ...
    public function updateLanguageType(){
        ...
        $reply=$this->modAdmin->updatePlaceType($data,$data['id']);
                ...
                redirect('admin/viewLanguageType');
        ...
        if (!empty($data['name'])  && !empty($data['nationality']) &&!empty($data['address']) && !empty($data['mobailenumber']) && !empty($data['area']) &&!empty($data['user_name']) && !empty($data['pic'] && !empty($data['password'])) ) {
            ...
            if ($chkRegister==0) {
                ...
                if ($addGuide) {
                    ...
                    $this->session->set_flashdata('class','alert-danger');
                    $this->session->set_flashdata('message','Sorry Your Provided Mobaile Number or User Name Alredy Used');
                    redirect('admin/addGuide');
                }
        }
        else{
